...
public class PenSample3_1_SelectObject extends Activity {

    private final int CONTEXT_MENU_DELETE_ID = 10;
    private final int CONTEXT_MENU_COPY_ID = 11;
    private final int CONTEXT_MENU_PASTE_ID = 12;
    ...
    private ArrayList<SpenObjectBase> copyList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSpenSurfaceView.setLongPressListener(mLongPressListener);
        ...
        pasteMenu.add(new SpenContextMenuItemInfo(CONTEXT_MENU_PASTE_ID, "Paste", true));
        mPasteContextMenu = new SpenContextMenu(mContext, spenViewLayout, pasteMenu,
                new SpenContextMenu.ContextMenuListener() {
                    @Override
    ...
    private final SpenLongPressListener mLongPressListener = new SpenLongPressListener() {

        @Override
        public void onLongPressed(MotionEvent event) {
            mSpenSurfaceView.closeControl();
            RectF rectfRelative = new RectF(event.getX(), event.getY(), event.getX() + 1, event.getY() + 1);
            RectF rectfAbsolute = convertRelative(rectfRelative);
            offsetX = rectfAbsolute.left;
            offsetY = rectfAbsolute.top;

            ArrayList<SpenObjectBase> aa = mSpenPageDoc.findObjectAtPosition(SpenPageDoc.FIND_TYPE_ALL, offsetX,
                    offsetY);
            if (copyList != null && copyList.size() != 0 && aa.size() == 0) {
                mPasteContextMenu.setRect(new Rect((int) rectfRelative.left, (int) rectfRelative.top,
                        (int) rectfRelative.right, (int) rectfRelative.bottom));
    ...
    private void pasteObjects() {
        mSpenNoteDoc.reviseObjectList(copyList);
        RectF boundRect = copyList.get(0).getRect();
        for (int i = 1; i < copyList.size(); i++) {
            RectF childRect = copyList.get(i).getRect();
        ...
        for (int j = 0; j < copyList.size(); j++) {
            SpenObjectBase childObject = copyList.get(j);
        ...
        // RectF _rectf = pasteList.get(i).getRect();
        // _rectf.offset(offsetX, offsetY);
        // pasteList.get(i).setRect(_rectf, false);
        // }
    ...
    private final SpenControlListener mControlListener = new SpenControlListener() {
        ...
        public boolean onMenuSelected(ArrayList<SpenObjectBase> objectList, int itemId) {
            ...
            case CONTEXT_MENU_COPY_ID:
                copyList = new ArrayList<SpenObjectBase>();
                for (int i = 0; i < mSpenPageDoc.getSelectedObjectCount(); i++) {
                    SpenObjectBase tmp = mSpenPageDoc.getSelectedObject().get(i);
                    switch (tmp.getType()) {
                    case SpenObjectBase.TYPE_STROKE:
                        SpenObjectBase stroke = new SpenObjectStroke();
                        stroke.copy(tmp);
        ...
        public boolean onCreated(ArrayList<SpenObjectBase> objectList, ArrayList<Rect> relativeRectList,
            ...
            menu.add(new SpenContextMenuItemInfo(CONTEXT_MENU_COPY_ID, "Copy", true));
